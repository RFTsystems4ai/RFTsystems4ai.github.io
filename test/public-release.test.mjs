...
test('public tree contains no protected-source breadcrumbs, payment/runtime dependencies or private signing material',()=>{const forbidden=[/huggingface/i,/sources-private/i,/private-integrations/i,/ai-verification-workshop/i,/BEGIN PRIVATE KEY/i,/RFTSYSTEMS4AI_RECEIPT_PRIVATE_KEY/i,/stripe/i,/vercel/i,/cloudflare/i];const files=walk(root).filter(f=>!f.includes(`${path.sep}.git${path.sep}`));for(const file of files){const text=fs.readFileSync(file,'utf8');for(const pattern of forbidden)assert.doesNotMatch(text,pattern,`${path.relative(root,file)} contains forbidden publication term ${pattern}`);}});
